...
class PacmanDeepQAgent(PacmanQAgent):
    ...
    def compute_q_targets(self, minibatch, network = None, target_network=None, doubleQ=False):
        ...
        bonus = 1.0 / (2.0 * np.sqrt(self.counts[states_np[:, 0], states_np[:, 1]] / 100.0))

        q_next = network.run(next_states)
        res_next = target_network.run(next_states)
        argmax = np.argmax(q_next.data, axis=1)
        q_argmax = np.choose(argmax, res_next.data.T)

        q_diff = rewards + (1.0 - dones) * self.discount * q_argmax + bonus
        Q_target[np.arange(num_batch), actions] = q_diff
